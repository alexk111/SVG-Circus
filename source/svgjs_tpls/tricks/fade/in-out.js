x=function(actor, progress) {
  ...
  */
  ...
  if(progress<=__timing1) {
    node.setAttribute("opacity",progress*(actor.opacity/__timing1));
  } else if(progress>=__timing2) {
    node.setAttribute("opacity",actor.opacity-(progress-__timing2)*(actor.opacity/(1-__timing2)));
  } else {
    node.setAttribute("opacity",actor.opacity);
  }
};